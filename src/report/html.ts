...
function buildTicketGroups(groups: TicketGroup[]): string {
  return groups
    .map((group) => {
      const priorityBadge = group.priority !== undefined && group.priority > 0
        ? `<span class="badge" style="background:${PRIORITY_BADGE_COLOR[group.priority] ?? '#9ca3af'}">${escapeHtml(PRIORITY_LABELS[group.priority] ?? `P${group.priority}`)}</span>`
        : '';
      const statusBadge = group.status
        ? `<span class="badge badge-status">${escapeHtml(group.status)}</span>`
        : '';
      const titleHtml = group.url
        ? `<a href="${escapeHtml(group.url)}" target="_blank" rel="noopener">${escapeHtml(group.identifier)}: ${escapeHtml(group.title)}</a>`
        : `${escapeHtml(group.identifier)}: ${escapeHtml(group.title)}`;
      const cycleHtml = group.cycleNumber !== undefined
        ? `<span class="cycle">Sprint ${group.cycleNumber}${group.cycleTitle ? `: ${escapeHtml(group.cycleTitle)}` : ''}</span>`
        : '';

      const rows = group.commits
        .map((c) => {
          const stat = `+${c.diffStat.insertions}/-${c.diffStat.deletions}`;
          return `<tr>
            <td><code>${escapeHtml(c.sha.slice(0, 7))}</code></td>
      ...
      return `<div class="ticket-group">
        <div class="ticket-header">
          <span class="ticket-title">${titleHtml}</span>
          ${priorityBadge}${statusBadge}${cycleHtml}
        </div>
        <table>
          <thead><tr><th>SHA</th><th>Message</th><th>Changes</th></tr></thead>
          <tbody>${rows}</tbody>
...
const CSS = `
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .ticket-header { display: flex; align-items: center; flex-wrap: wrap; gap: 8px; margin-bottom: 10px; font-size: 0.95rem; font-weight: 600; color: #111827; }
  .ticket-header a { color: #1d4ed8; text-decoration: none; }
  .ticket-header a:hover { text-decoration: underline; }
  .badge { display: inline-block; padding: 2px 8px; border-radius: 9999px; font-size: 0.75rem; font-weight: 600; color: #fff; }
  .badge-status { background: #374151; }
  .cycle { font-size: 0.8rem; color: #6b7280; font-weight: 400; }
`;
...
<body>
  <div class="page">
    ...
    ${report.ticketGroups && report.ticketGroups.length > 0 ? `
    <div class="card">
      <h2>By Issue</h2>
      ${buildTicketGroups(report.ticketGroups)}
    </div>` : ''}

    <footer>Generated by work-summary &bull; ${escapeHtml(new Date().toISOString())}</footer>
  </div>
</body>
</html>`;
}
