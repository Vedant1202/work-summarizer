    ...
    <Section title="Trigger Deployment">
      ...
      {result && (
        <div style={successBox}>
          ...
          {result.previewUrl && <div style={{ marginTop: 4 }}>Preview URL: <a href={result.previewUrl} target="_blank" rel="noreferrer" style={{ color: '#276749' }}>{result.previewUrl}</a></div>}
          <div style={{ marginTop: 4, fontSize: 12, color: '#555' }}>Deployment is running in the background and will be saved to history on completion.</div>
        </div>
      )}
...
function StatusSection() {
  ...
  return (
    <Section title="Check Status">
      ...
      {result && (
        <div style={{ ...successBox, borderColor: statusColor.startsWith('#c5') ? '#feb2b2' : '#9ae6b4' }}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            <thead>
              <tr style={{ borderBottom: '1px solid #e5e5e5' }}>
                {['Date', 'Mode', 'Branch', 'Status', 'Location'].map((h) => (
